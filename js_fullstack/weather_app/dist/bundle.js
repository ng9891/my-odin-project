...
const DailyUI = (Weather) => {
  ...
  const _createDaily = (data) => {
    ...
    for (const {dt, weather, temp} of data) {
      ...
      if (tempUnit === 'f') newTemp = (0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(newTemp);
      tempSpan.textContent = `${Math.floor(newTemp)}${_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.degree}`;
...
const ForecastUI = (Weather) => {
  ...
  const _createSmallDetail = (date, temp, lowTemp, highTemp, feel) => {
    ...
    if (tempUnit === 'f') {
      temp = Math.floor((0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(temp));
      lowTemp = Math.floor((0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(lowTemp));
      highTemp = Math.floor((0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(highTemp));
      feel = Math.floor((0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(feel));
    } else {
  ...
  const _createHourlyForecast = (data) => {
    ...
    const createCard = ({date, iconID, condition, temp, precipitationChance}) => {
      ...
      conditionSpan.textContent = condition;
      let newTemp = temp;
      if (tempUnit === 'f') newTemp = (0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.celsiusToFahrenheit)(temp);
      tempSpan.textContent = `${Math.floor(newTemp)}${_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.degree}`;
...
const search = async () => {
  ...
  const results = await _weatherController__WEBPACK_IMPORTED_MODULE_1__["default"].geocode(searchInput.value);
  const {name, country, lat, lon} = results.data[0];
  const data = await _weatherController__WEBPACK_IMPORTED_MODULE_1__["default"].getData(lat, lon);

  const weather = (0,_weather__WEBPACK_IMPORTED_MODULE_0__["default"])(name, country, data);
  forecast = forecast ? forecast.setWeather(weather) : (0,_ForecastUI__WEBPACK_IMPORTED_MODULE_2__["default"])(weather);
  daily = daily ? daily.setWeather(weather) : (0,_DailyUI__WEBPACK_IMPORTED_MODULE_3__["default"])(weather);
...
const main = () => {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      const reverse = await _weatherController__WEBPACK_IMPORTED_MODULE_1__["default"].reverseGeocode(lat, lon);
      const {name, country} = reverse[0];
      const data = await _weatherController__WEBPACK_IMPORTED_MODULE_1__["default"].getData(lat, lon);
      console.log(data);
      const newWeather = (0,_weather__WEBPACK_IMPORTED_MODULE_0__["default"])(name, country, data);

      forecast = (0,_ForecastUI__WEBPACK_IMPORTED_MODULE_2__["default"])(newWeather);
      daily = (0,_DailyUI__WEBPACK_IMPORTED_MODULE_3__["default"])(newWeather);
    });
  }else{
...
const WeatherController = (() => {
  ...
  return {
    ...
    reverseGeocode: (0,_helpers_helper__WEBPACK_IMPORTED_MODULE_0__.handleError)(reverseGeocodeUnsafe),
